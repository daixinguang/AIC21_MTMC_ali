...
def viz_mcmt(args,cam,cam_track):
    ...
    cam_dir = os.path.join(args.data_path,args.mode,args.scenes,cam)
    cap = cv2.VideoCapture(os.path.join(cam_dir,"vdo.avi"))
    out_dit = os.path.join(args.output_path,'mot',args.scenes)
    img_dir = os.path.join(args.output_path,'mot',args.scenes,'imgs')
    if not os.path.isdir(img_dir):
        os.makedirs(img_dir)
    out = cv2.VideoWriter(os.path.join(out_dit,"{}.mp4".format(cam)),
    ...
    while (state):
        if fr_id in cam_track:
            ...
            for pid,x,y,w,h in cam_track[fr_id]:
                ...
                clip = im[y:(y+h),x:(x+w)]
                cv2.imwrite(os.path.join(img_dir_out,'{}_{}_{}.jpg'.format(cam,pid,fr_id)),clip)
...
def main(args):
    ...
    for i,c in enumerate(cam_tracks):
        viz_mcmt(args,cam_paths[i],cam_tracks[c])
...
if __name__ == "__main__":
    ...
    args.mode, args.scenes = cfg.DET_SOURCE_DIR.split(os.sep)[-3:-1]
    main(args)
