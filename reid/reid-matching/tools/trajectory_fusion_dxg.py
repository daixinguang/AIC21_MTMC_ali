...
def parse_pt_ori(pt_file,zones):
    ...
    for line in lines:
        ...
        if tid not in mot_list:
            mot_list[tid] = dict()
        out_dict = lines[line]
        out_dict['zone'] = zones.get_zone(bbox) # dxg 这里获取对应cam的区域，可是valid没有对应图片
        mot_list[tid][fid] = out_dict
    return mot_list
...
if __name__ == '__main__':
    ...
    cfg.freeze()
    # scene_name = ['S02'] # dxg
    use_zone= cfg.USE_ZONE # dxg
    data_dir = cfg.DATA_DIR
    mode, scenes = cfg.DET_SOURCE_DIR.split(os.sep)[-3:-1] # dxg
    
    cam_ = []
    for cam in os.listdir(cfg.DET_SOURCE_DIR):
        cam_.append(int(cam[1:]))
    #### dxg ####
    # cam_.remove(5) # no_c005
    save_dir = f'./exp/viz/{mode}/{scenes}/trajectory/' # dxg
    ...
    for cam_path in cam_paths:
        ...
        out_new_mot(mot_list, new_mot_path) # 保存到cXX_mot_feat_break.pkl
